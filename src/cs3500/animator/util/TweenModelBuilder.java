...
public interface TweenModelBuilder<T> {
  ...
  /**
   * Change the x and y extents of this shapeDir from the specified extents to the
   * specified target extents. What these extents actually mean depends on the
   * shapeDir, but these are roughly the extents of the box enclosing the shapeDir
   */
  ...
  TweenModelBuilder<T> addRotate(String name, float fromDegree, float toDegree,
                                 int startTime, int endTime);
