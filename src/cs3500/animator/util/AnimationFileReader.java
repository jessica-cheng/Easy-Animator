 ...
 * file in the prescribed file format, and relies on a modelDir Builder interface.
 * The user of this class should create a modelDir Builder that implements this
 * interface.
 */
...
public class AnimationFileReader {
  ...
  /**
   * Read the animation file and use the Builder to build a modelDir.
   *
   * @param fileName the path of the file to be read
   * @param builder  the Builder used to build the modelDir
   * @param <T>      the type of modelDir
   * @return the modelDir
    ...
    while (sc.hasNext()) {
      ...
      switch (command) {
        ...
        case "rotate":
          ...
          builder.addRotate(rotateInfo.name,
                  rotateInfo.getFromDegree(),
                  rotateInfo.getToDegree(),
                  rotateInfo.getStart(),
                  rotateInfo.getEnd());
          break;
  ...
  private RotateInfo readRotateInfo(Scanner sc) throws
    ...
    while (!info.isAllInitialized()) {
      ...
      switch (command) {
        case "rotate":
          info.setFromDegree(sc.nextFloat());
          info.setToDegree(sc.nextFloat());
          break;
        case "name":
          info.setName(sc.next());
          break;
  ...
  class RotateInfo extends Inputable {
    ...
    RotateInfo() {
      ...
      valueFlags.put("name", false);
      valueFlags.put("fromDegree", false);
      valueFlags.put("toDegree", false);
      valueFlags.put("start", false);
      valueFlags.put("end", false);
    ...
    void setFromDegree(float x) {
      this.fromDegree = x;
      valueFlags.replace("fromDegree", true);
    }


    void setToDegree(float x) {
      this.toDegree = x;
      valueFlags.replace("toDegree", true);
    }
